...
export const CONDITIONAL_ARMOUR_RUNE_RULES_VERSION = 'basic-2026-09-16-v86'

/** 起点、导入声明、完整未来历史及未执行指引共用身份核对，不解析观察文本。 */
export function requiresConditionalArmourRuneProjectVersion(
  input: unknown,
  catalog: CraftCatalog,
): boolean {
  const augments = catalog.augments?.filter(isConditionalArmourRune) ?? []
