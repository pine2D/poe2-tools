...
const id = 'pob2:augment:["Legacy of Lifesprig","wand"]'
function fixture() {
  return {
    schemaVersion: 1,
...
it('旧版拒绝新未来，双来源指纹缺失或变化不能恢复', () => {
  ...
  expect(
    loadTargetWorkbenchProject(
      JSON.stringify({ ...project, rulesVersion: 'basic-2026-09-18-v123' }),
      catalog,
    ),
  ).toMatchObject({ ok: false, error: expect.stringContaining('v124') })
...
it('能力检测不执行访问器，不将既有同名报价升级，保留嵌套与导入声明', () => {
  expect(requiresWandRuneProjectVersion({ pricing: { 'augment:Legacy of Lifesprig': 1 } })).toBe(
    false,
  )
  expect(requiresWandRuneProjectVersion({ nested: { importedSockets: [id] } })).toBe(true)
  expect(requiresWandRuneProjectVersion({ nested: { kind: 'socket', augmentId: id } })).toBe(true)
  const getter = Object.defineProperty({}, 'sockets', {
    get() {
      throw Error('不可执行')
    },
  })
  expect(requiresWandRuneProjectVersion(getter)).toBe(false)
})
