...
export function loadTargetWorkbenchProject(
  ...
  return value !== null &&
    ...
    (value.rulesVersion === TARGET_CRAFT_RULES_VERSION ||
      ...
      value.rulesVersion === CASTER_SOUL_CORE_RULES_VERSION ||
      value.rulesVersion === STAFF_RUNE_RULES_VERSION ||
      value.rulesVersion === RUNESEEKER_RULES_VERSION ||
      value.rulesVersion === WAND_RUNE_RULES_VERSION ||
      value.rulesVersion === RING_LICH_ECHOES_RULES_VERSION ||
      value.rulesVersion === SPENDING_STRATEGY_RULES_VERSION ||
      value.rulesVersion === TIERED_MASTERWORK_RULES_VERSION ||
      value.rulesVersion === INFUSED_CATALYST_RULES_VERSION ||
      value.rulesVersion === BLACKBLOODED_ECHOES_RULES_VERSION ||
      value.rulesVersion === ANCIENT_RIB_ECHOES_RULES_VERSION ||
      value.rulesVersion === ORDINARY_ATTRIBUTE_ESSENCE_RULES_VERSION ||
      value.rulesVersion === JEWEL_VAAL_RULES_VERSION ||
...
export function reuseTargetCraftPlan(
  ...
  if (source.fluxCatalogSignature !== undefined) {
    if (
      next.rulesVersion !== CASTER_SOUL_CORE_RULES_VERSION &&
      next.rulesVersion !== STAFF_RUNE_RULES_VERSION &&
      next.rulesVersion !== RUNESEEKER_RULES_VERSION &&
      next.rulesVersion !== WAND_RUNE_RULES_VERSION &&
      next.rulesVersion !== RING_LICH_ECHOES_RULES_VERSION &&
      next.rulesVersion !== SPENDING_STRATEGY_RULES_VERSION &&
      next.rulesVersion !== TIERED_MASTERWORK_RULES_VERSION &&
      next.rulesVersion !== INFUSED_CATALYST_RULES_VERSION &&
      next.rulesVersion !== BLACKBLOODED_ECHOES_RULES_VERSION &&
      next.rulesVersion !== ANCIENT_RIB_ECHOES_RULES_VERSION &&
      next.rulesVersion !== ORDINARY_ATTRIBUTE_ESSENCE_RULES_VERSION &&
      next.rulesVersion !== JEWEL_VAAL_RULES_VERSION &&
  ...
  if (
    ...
    next.rulesVersion === RUNESEEKER_RULES_VERSION ||
    next.rulesVersion === WAND_RUNE_RULES_VERSION ||
    requiresRuneseekerProjectVersion(next) ||
    requiresCasterSoulCoreProjectVersion(next) ||
    requiresStaffRuneProjectVersion(next) ||
    requiresWandRuneProjectVersion(next) ||
    next.rulesVersion === RING_LICH_ECHOES_RULES_VERSION ||
    requiresRingLichEchoesProjectVersion(next, catalog) ||
    next.rulesVersion === SPENDING_STRATEGY_RULES_VERSION ||
    requiresSpendingProjectVersion(next) ||
    next.rulesVersion === TIERED_MASTERWORK_RULES_VERSION ||
    requiresTieredMasterworkProjectVersion(next, catalog) ||
    next.rulesVersion === INFUSED_CATALYST_RULES_VERSION ||
    requiresInfusedCatalystProjectVersion(next, catalog) ||
    next.rulesVersion === BLACKBLOODED_ECHOES_RULES_VERSION ||
    requiresBlackbloodedEchoesProjectVersion(next) ||
    next.rulesVersion === ANCIENT_RIB_ECHOES_RULES_VERSION ||
    requiresAncientRibEchoesProjectVersion(next) ||
    next.rulesVersion === ORDINARY_ATTRIBUTE_ESSENCE_RULES_VERSION ||
    requiresOrdinaryAttributeEssenceProjectVersion(next) ||
    next.rulesVersion === JEWEL_VAAL_RULES_VERSION ||
    requiresJewelVaalProjectVersion(next) ||
    next.rulesVersion === DEFENCE_ESSENCE_RULES_VERSION ||
    ...
    next.rulesVersion =
      ...
      requiresCasterSoulCoreProjectVersion(next)
        ? CASTER_SOUL_CORE_RULES_VERSION
        : next.rulesVersion === STAFF_RUNE_RULES_VERSION || requiresStaffRuneProjectVersion(next)
          ? STAFF_RUNE_RULES_VERSION
          : next.rulesVersion === RUNESEEKER_RULES_VERSION || requiresRuneseekerProjectVersion(next)
            ? RUNESEEKER_RULES_VERSION
            : next.rulesVersion === WAND_RUNE_RULES_VERSION || requiresWandRuneProjectVersion(next)
  ...
  if (
    ...
    requiresRetainedCatalystProjectVersion(
      ...
      catalog,
      current.value.project.rulesVersion === CASTER_SOUL_CORE_RULES_VERSION ||
        current.value.project.rulesVersion === STAFF_RUNE_RULES_VERSION ||
        current.value.project.rulesVersion === RUNESEEKER_RULES_VERSION ||
        current.value.project.rulesVersion === WAND_RUNE_RULES_VERSION ||
        current.value.project.rulesVersion === RING_LICH_ECHOES_RULES_VERSION ||
        current.value.project.rulesVersion === SPENDING_STRATEGY_RULES_VERSION ||
        current.value.project.rulesVersion === TIERED_MASTERWORK_RULES_VERSION ||
        current.value.project.rulesVersion === INFUSED_CATALYST_RULES_VERSION,
    )
  )
    next.rulesVersion = RETAINED_CATALYST_RULES_VERSION
  if (
