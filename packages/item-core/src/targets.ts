...
export function analyzeCraftTargets(
  ...
  return analyzeCraftTargetContext(
    catalog,
    state,
    ids,
    targetValues,
...
/** 包内共用分析：原生调用方先完整验证定义，目录候选仍执行实际制作规则。 */
export function analyzeCraftTargetContext(
  catalog: CraftCatalog,
  state: CraftState,
  ids: readonly string[],
  targetValues: readonly CraftTargetValues[] = [],
  ...
  if (!definitions && fracturedTargetId !== undefined) {
  ...
  const fractureMemberIds = new Set(
    definitions
      ? definitions.targets
          .filter((target) => target.targetId === definitions.fracturedTargetId)
          .flatMap((target) => [
            target.modId,
            ...(definitions.alternatives.find((entry) => entry.targetId === target.targetId)
              ?.modIds ?? []),
          ])
      : [
          fracturedTargetId,
  ...
  const inspectTarget = (
    ...
    const goal = definitions
      ? definitions.values.find((entry) => entry.targetId === targetId && entry.modId === modId)
      : values.value.find((entry) => entry.modId === modId)
    const instances = inspectCraftTargetInstances(catalog, current, modId, goal, fractureRequired)
    const assigned = progress?.matches.find((match) => match.targetId === targetId)
    const selected =
      instances.find((entry) => entry.index === assigned?.affixIndex) ??
      instances.find((entry) => entry.matched) ??
      instances[0]
  ...
  const targets: CraftAdvice['targets'] = validated.value.map((modId, index) => {
    const targetId = definitions?.targets[index]?.targetId
    const fractureRequired = definitions
      ? targetId === definitions.fracturedTargetId
      : modId === fracturedTargetId
    const primary = inspectTarget(modId, fractureRequired, targetId)
    const accepted = definitions
      ? definitions.alternatives.find((entry) => entry.targetId === targetId)
      : alternatives.find((entry) => entry.targetModId === modId)
    if (accepted === undefined) return primary
    const members = [
      primary,
      ...accepted.modIds.map((id) => inspectTarget(id, fractureRequired, targetId)),
    ]
  ...
  const presence = definitions
    ? evaluateTargetDefinitions(
        catalog,
        {
          ...current,
          affixes: current.affixes.filter(
            (_, index) => !progress?.matches.some((match) => match.affixIndex === index),
          ),
        },
        {
          targets: definitions.targets.filter(
            (target) => !progress?.matches.some((match) => match.targetId === target.targetId),
          ),
          alternatives: definitions.alternatives,
          values: [],
        },
      )
    : undefined
  if (progress && definitions)
    for (const [index, target] of targets.entries()) {
      target.present = [...progress.matches, ...(presence?.matches ?? [])].some(
        (match) => match.targetId === definitions.targets[index]?.targetId,
      )
      const match = progress.matches.find(
        (match) => match.targetId === definitions.targets[index]?.targetId,
      )
      target.matched = match !== undefined
  ...
  if (
    progress
      ? progress.satisfied && implicitAnalysis.value.every((target) => target.matched)
  ...
  const missing = definitions
    ? [
        ...new Set([
          ...missingTargets,
          ...definitionFluxSourceModIds(catalog, current, missingTargets),
        ]),
      ]
    : missingTargets
  ...
  const unmetNumericIds = numericTargets
    ...
    .filter((target) => {
      if (definitions || minimumTargetCount === undefined || minimumTargetCount === ids.length)
        return true
  ...
  const explicitRollsPossible = current.affixes.every((affix) => {
    ...
    const mod = byId.get(affix.modId)
    if (definitions && mod)
      return minimumCraftTargetRolls(catalog, current, mod, undefined, affix.lines) !== null
    return (
      (mod !== undefined &&
        minimumCraftTargetRolls(
          catalog,
    ...
    for (const removed of removals) {
      ...
      const targetModIds = missing.filter((id) => {
        ...
        return (
          mod !== undefined &&
          (definitions
            ? definitionModRollOptions(catalog, next, definitions, mod).length > 0
            : minimumCraftTargetRolls(
                catalog,
                next,
                mod,
      ...
      const lost = new Set(
        definitions
          ? specialTargetContext(catalog, current, definitions).affected(prepared.value.state)
          : lostCraftTargetIds(
              catalog,
              current,
