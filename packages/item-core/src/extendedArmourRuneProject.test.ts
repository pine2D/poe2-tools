...
const wardId = 'pob2:augment:["Rebirth Rune","armour"]'
const version = 'basic-2026-09-16-v84'
...
it('v2–v83 均拒绝未来结界符文、未执行嵌套指引和仅报价', () => {
  ...
  for (let n = 2; n <= 83; n++) {
    ...
    for (const extra of [
      ...
      { pricing: { unit: 'divine', prices: { 'augment:Rebirth Rune': 1 } } },
    ])
      expect(read(JSON.stringify({ ...input, ...extra }), real), `v${n}`).toMatchObject({
        ok: false,
        error: expect.stringContaining('v84'),
      })
  }
})
...
const chargingId = 'pob2:augment:["Warding Rune of Reinforcement","armour"]'
...
function socketProject() {
  return {
    ...
    pricing: { unit: 'divine', prices: { 'augment:Rebirth Rune': 2 } },
  }
}
it('v84保留全未来覆盖历史、每个游标及报价', () => {
...
it('仅报价与未执行嵌套条件指引触发v84，观察文本不触发能力', () => {
  ...
  for (const input of [
    ...
    expect(requiresExtendedArmourRuneProjectVersion(input, catalog)).toBe(true)
    expect(must(loadTargetWorkbenchProject(JSON.stringify(input), catalog)).project).toEqual(input)
  }
  expect(
    requiresExtendedArmourRuneProjectVersion({ sourceText: JSON.stringify(source) }, catalog),
  ).toBe(false)
})
it('沿用新指引升至v84；沿用旧指引不降级、不截断未来、不覆盖报价', () => {
  const strategy = {
    maxSteps: 4,
    rules: [
      {
  ...
  const old = {
    ...socketProject(),
    rulesVersion: 'basic-2026-09-16-v83',
    operations: [],
    pricing: undefined,
  }
...
it('五种真实防具符文覆盖起点、导入声明、未来、嵌套策略和材料名报价', () => {
  const augments =
    catalog.augments?.filter(
      (a) =>
        a.category === 'armour' &&
        /^(?:(?:(?:Lesser|Greater|Perfect) )?Rebirth Rune|Warding Rune of Reinforcement)$/.test(
          a.name,
        ),
    ) ?? []
  expect(augments).toHaveLength(5)
  for (const augment of augments) {
    for (const input of [
      { initialState: { sockets: [augment.id] } },
      { importedSockets: [augment.id] },
      { operations: [{ kind: 'socket', augmentId: augment.id }], cursor: 0 },
      { strategy: { flow: { nested: [{ action: { kind: 'socket', augmentId: augment.id } }] } } },
      { pricing: { unit: 'divine', prices: { [`augment:${augment.name}`]: 0 } } },
    ])
      expect(requiresExtendedArmourRuneProjectVersion(input, catalog), augment.name).toBe(true)
  }
  ...
  expect(requiresExtendedArmourRuneProjectVersion(accessor, catalog)).toBe(false)
  const circular: { child?: unknown } = {}
  circular.child = circular
  expect(requiresExtendedArmourRuneProjectVersion(circular, catalog)).toBe(false)
})
it('v84接收v83锻造指引保留v84并补齐完整签名', () => {
  const loaded = {
    ...catalog,
    runeforging: JSON.parse(readFileSync('data/craft/runeforging.json', 'utf8')),
  }
