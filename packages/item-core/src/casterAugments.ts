...
import { CASTER_SOUL_CORES, CASTER_SOUL_SCALABILITY } from './casterSoulCoreData'
...
const runes = [...WAND_RUNES, ...STAFF_RUNES, ...CASTER_SOUL_CORES]
const records = new Map(runes.map((a) => [a.id, a]))
export function isCasterAugmentId(id: unknown): boolean {
...
const metadata = {
  ...WAND_RUNE_SCALABILITY,
  ...STAFF_RUNE_SCALABILITY,
  ...CASTER_SOUL_SCALABILITY,
} as Record<string, { scalable: boolean; formats: string[] }[]>

/** 身份和固定参数完整核对；有效副本只允许可缩放数值改变。 */
export function isCasterAugment(augment: CatalogAugment, effective = false): boolean {
...
export function casterAugmentFits(
...
export function isCasterAugmentLine(line: string): boolean {
  return (
    !/^\d+% increased Critical Hit Chance for Spells$/.test(line) && templates.has(template(line))
  )
}
/** 只有同一属性的可叠加贡献合计；其他条件、负作用与重复行逐行核对。 */
export function casterAugmentSourceMatches(
  ...
  const select = (lines: readonly string[]) => {
    ...
    let spellLevels = 0,
      penetration = 0
    for (const line of lines.filter(isCasterAugmentLine)) {
...
export function casterAugmentLimitKey(augment: CatalogAugment): string | null {
  return isCasterAugment(augment) && augment.limit === 1 ? (augment.limitId ?? augment.name) : null
}
export function casterAugmentSocketError(
  catalog: CraftCatalog,
  state: CraftState,
  index: number,
  id: string,
  ...
  const key = augment && casterAugmentLimitKey(augment)
  if (!key) return null
  return (state.sockets ?? []).some((old, i) => {
    const other = catalog.augments?.find((a) => a.id === old)
    return i !== index && other && casterAugmentLimitKey(other) === key
  })
    ? '该镶嵌物已达到本件限量；遗产符文共用限量，请替换已有同组孔位。'
    : null
}
