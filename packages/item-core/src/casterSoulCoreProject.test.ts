...
const id = 'pob2:augment:["Soul Core of Vizoma","staff"]'
function fixture() {
  return {
    schemaVersion: 1,
    rulesVersion: 'basic-2026-09-24-v127',
...
it('旧版拒绝新未来，双来源指纹缺失或变化不能恢复', () => {
  ...
  expect(
    loadTargetWorkbenchProject(
      JSON.stringify({ ...project, rulesVersion: 'basic-2026-09-18-v126' }),
      catalog,
    ),
  ).toMatchObject({ ok: false, error: expect.stringContaining('v127') })
...
it('能力检测不执行访问器，不将既有同名报价升级，保留嵌套与导入声明', () => {
  expect(
    requiresCasterSoulCoreProjectVersion({ pricing: { 'augment:Soul Core of Vizoma': 1 } }),
  ).toBe(false)
  expect(requiresCasterSoulCoreProjectVersion({ nested: { importedSockets: [id] } })).toBe(true)
  expect(requiresCasterSoulCoreProjectVersion({ nested: { kind: 'socket', augmentId: id } })).toBe(
    true,
  )
  ...
  expect(requiresCasterSoulCoreProjectVersion(getter)).toBe(false)
})
