...
export const OFFHAND_IDOL_RULES_VERSION = 'basic-2026-09-18-v113'

/** 仅精确雕像身份声明启用新能力；完整遍历未来与未执行指引，忽略报价及观察文本。 */
export function requiresOffhandIdolProjectVersion(input: unknown): boolean {
  const includes = (value: unknown) =>
    Array.isArray(value) &&
    Object.values(Object.getOwnPropertyDescriptors(value)).some((descriptor) =>
      isOffhandIdolId(descriptor.value),
    )
  return hasProjectCapability(
    input,
    (properties) =>
      (properties.kind?.value === 'socket' && isOffhandIdolId(properties.augmentId?.value)) ||
