...
import {
  ...
  ordinaryAttributeEssenceProjectCapabilityError,
...
import { requiresWandRuneProjectVersion, WAND_RUNE_RULES_VERSION } from './wandRuneProjectVersion'
...
export interface TargetCraftProject
  ...
  rulesVersion:
    ...
    | typeof CASTER_SOUL_CORE_RULES_VERSION
    | typeof STAFF_RUNE_RULES_VERSION
    | typeof RUNESEEKER_RULES_VERSION
    | typeof WAND_RUNE_RULES_VERSION
    | typeof RING_LICH_ECHOES_RULES_VERSION
    | typeof SPENDING_STRATEGY_RULES_VERSION
    | typeof TIERED_MASTERWORK_RULES_VERSION
    | typeof INFUSED_CATALYST_RULES_VERSION
    | typeof BLACKBLOODED_ECHOES_RULES_VERSION
    | typeof ANCIENT_RIB_ECHOES_RULES_VERSION
    | typeof ORDINARY_ATTRIBUTE_ESSENCE_RULES_VERSION
    | typeof JEWEL_VAAL_RULES_VERSION
    ...
    | typeof HELMET_BOOT_IDOL_RULES_VERSION
    | typeof GLOVE_IDOL_RULES_VERSION
    | typeof SCEPTRE_AUGMENT_RULES_VERSION
    | typeof SPECIAL_MARTIAL_RUNE_RULES_VERSION
    | typeof TALISMAN_CRAFT_RULES_VERSION
    | typeof FLASK_CRAFT_RULES_VERSION
    | typeof AMULET_CATALYST_RULES_VERSION
    | typeof AMULET_SKILL_LEVEL_RULES_VERSION
    | typeof AMULET_SKILL_SOCKETS_RULES_VERSION
...
export function parseTargetCraftProject(
  ...
  // 已超出合法历史上限时，先拒绝再扫描各版本能力，避免无意义的全树重复遍历。
  if (
    record(original) &&
    Array.isArray(original.operations) &&
    original.operations.length > MAX_CRAFT_PROJECT_OPERATIONS
  )
    return { ok: false, error: '演练操作列表无效或超过 1000 步。' }
  if (
    !record(original) ||
    ...
    return {
      ok: false,
      error:
        '目标项目必须使用精确的 v74、v75、v76、v77、v78、v79、v80、v81、v82、v83、v84、v85、v86、v87、v88、v89、v90、v91、v92、v93、v94、v95、v96、v97、v98、v99、v100、v101、v102、v103、v104、v105、v106、v107、v108、v109、v110 、v111、v112、v113 、v114、v115 、v116 、v117 、v118 、v119 、v120 、v121 、v122 、v123 或 v124、v125、v126、v127 规则版本。',
    }
  const casterSoulCores = original.rulesVersion === CASTER_SOUL_CORE_RULES_VERSION
  if (!casterSoulCores && requiresCasterSoulCoreProjectVersion(original))
    return { ok: false, error: '施法武器魂核必须使用 v127 项目。' }
  const staffRunes = casterSoulCores || original.rulesVersion === STAFF_RUNE_RULES_VERSION
  if (!staffRunes && requiresStaffRuneProjectVersion(original))
    return { ok: false, error: '长杖专属符文必须使用 v126 项目。' }
  const runeseeker = staffRunes || original.rulesVersion === RUNESEEKER_RULES_VERSION
  if (!runeseeker && requiresRuneseekerProjectVersion(original))
    return { ok: false, error: '符文间增效和扩展容量必须使用 v125 项目。' }
  const wandRunes = runeseeker || original.rulesVersion === WAND_RUNE_RULES_VERSION
  if (!wandRunes && requiresWandRuneProjectVersion(original))
    return { ok: false, error: '法杖专属符文必须使用 v124 项目。' }
  const ringLichEchoes = wandRunes || original.rulesVersion === RING_LICH_ECHOES_RULES_VERSION
  if (!ringLichEchoes && requiresRingLichEchoesProjectVersion(original, catalog))
    return { ok: false, error: '戒指两族回响必须使用 v123 项目。' }
  const spendingStrategy =
    ringLichEchoes || original.rulesVersion === SPENDING_STRATEGY_RULES_VERSION
  if (!spendingStrategy && requiresSpendingProjectVersion(original))
    return { ok: false, error: '已用材料费用条件必须使用 v122 项目。' }
  const tieredMasterwork =
    spendingStrategy || original.rulesVersion === TIERED_MASTERWORK_RULES_VERSION
  if (!tieredMasterwork && requiresTieredMasterworkProjectVersion(original, catalog))
    return { ok: false, error: '低阶符文升级及对应指引必须使用 v121 项目。' }
  const infusedCatalyst =
    tieredMasterwork || original.rulesVersion === INFUSED_CATALYST_RULES_VERSION
  if (!infusedCatalyst && requiresInfusedCatalystProjectVersion(original, catalog))
    return { ok: false, error: '首饰已有注能品质必须使用 v120 项目。' }
  const blackbloodedEchoes =
    infusedCatalyst || original.rulesVersion === BLACKBLOODED_ECHOES_RULES_VERSION
  if (!blackbloodedEchoes && requiresBlackbloodedEchoesProjectVersion(original))
    return { ok: false, error: '黑血项链回响必须使用 v119 项目。' }
  const ancientRibEchoes =
    blackbloodedEchoes || original.rulesVersion === ANCIENT_RIB_ECHOES_RULES_VERSION
  if (!ancientRibEchoes && requiresAncientRibEchoesProjectVersion(original))
    return { ok: false, error: '远古肋骨与回响组合必须使用 v118 项目。' }
  const ordinaryAttributeEssences =
    ancientRibEchoes || original.rulesVersion === ORDINARY_ATTRIBUTE_ESSENCE_RULES_VERSION
  if (!ordinaryAttributeEssences) catalog = withoutOrdinaryAttributeEssences(catalog)
  if (!ordinaryAttributeEssences && requiresOrdinaryAttributeEssenceProjectVersion(original))
    return { ok: false, error: '普通属性精华操作和指引必须使用 v117 项目。' }
  const attributeEssenceError = ordinaryAttributeEssences
    ? ordinaryAttributeEssenceProjectCapabilityError(original, catalog)
    : null
  if (attributeEssenceError) return { ok: false, error: attributeEssenceError }
  const jewelVaal = ordinaryAttributeEssences || original.rulesVersion === JEWEL_VAAL_RULES_VERSION
  if (!jewelVaal && requiresJewelVaalProjectVersion(original))
    return { ok: false, error: '珠宝瓦尔增删词缀必须使用 v116 项目。' }
  const defenceEssences = jewelVaal || original.rulesVersion === DEFENCE_ESSENCE_RULES_VERSION
  ...
  if (!offhandIdols && requiresOffhandIdolProjectVersion(original))
    return {
      ok: false,
      error: '副手雕像必须使用 v113 项目，包括起点、导入声明、完整未来和未执行指引。',
    }
  const bodyIdols = offhandIdols || original.rulesVersion === BODY_IDOL_RULES_VERSION
  if (!bodyIdols && requiresBodyIdolProjectVersion(original))
    return {
      ok: false,
      error: '胸甲雕像必须使用 v112 项目，包括起点、导入声明、完整未来和未执行指引。',
    }
  const helmetBootIdols = bodyIdols || original.rulesVersion === HELMET_BOOT_IDOL_RULES_VERSION
  if (!helmetBootIdols && requiresHelmetBootIdolProjectVersion(original))
    return {
      ok: false,
      error: '头盔和鞋部雕像必须使用 v111 项目，包括起点、导入声明、完整未来和未执行指引。',
    }
  const gloveIdols = helmetBootIdols || original.rulesVersion === GLOVE_IDOL_RULES_VERSION
  if (!gloveIdols && requiresGloveIdolProjectVersion(original))
    return {
      ok: false,
      error: '手套雕像必须使用 v110 项目，包括起点、导入声明、完整未来和未执行指引。',
    }
  const sceptreAugments = gloveIdols || original.rulesVersion === SCEPTRE_AUGMENT_RULES_VERSION
  if (!sceptreAugments && requiresSceptreAugmentProjectVersion(original))
    return { ok: false, error: '权杖镶嵌必须使用 v109 项目，包括孔位声明、完整未来和未执行指引。' }
  const specialMartialRunes =
    sceptreAugments || original.rulesVersion === SPECIAL_MARTIAL_RUNE_RULES_VERSION
  if (!specialMartialRunes && requiresSpecialMartialRuneProjectVersion(original))
    return {
      ok: false,
      error: '专属攻击武器符文必须使用 v108 项目，包括起点、导入声明、完整未来、指引和报价。',
    }
  const talismans = specialMartialRunes || original.rulesVersion === TALISMAN_CRAFT_RULES_VERSION
  if (!talismans && requiresTalismanProjectVersion(original, catalog))
    return {
      ok: false,
      error: '普通魔符制作必须使用 v107 项目，包括起点、目标、完整未来和未执行指引。',
    }
  const flasks = talismans || original.rulesVersion === FLASK_CRAFT_RULES_VERSION
  if (!flasks && requiresFlaskProjectVersion(original, catalog))
    return {
      ok: false,
      error: '药剂制作必须使用 v106 项目，包括起点、目标、完整未来和未执行指引。',
    }
  const amuletCatalyst = flasks || original.rulesVersion === AMULET_CATALYST_RULES_VERSION
  if (!amuletCatalyst && requiresAmuletCatalystProjectVersion(original))
    return {
      ok: false,
      error: '技能项链催化品质必须使用 v105 项目，包括起点、完整未来和嵌套未执行状态。',
    }
  const amuletSkillLevel =
    amuletCatalyst || original.rulesVersion === AMULET_SKILL_LEVEL_RULES_VERSION
  if (!amuletSkillLevel && requiresAmuletSkillLevelProjectVersion(original))
    return {
      ok: false,
      error: '技能项链等级制作必须使用 v104 项目，包括起点、完整未来、目标和未执行指引。',
    }
  const amuletSkillSockets =
    amuletSkillLevel || original.rulesVersion === AMULET_SKILL_SOCKETS_RULES_VERSION
  if (!amuletSkillSockets && requiresAmuletSkillSocketsProjectVersion(original))
    return {
      ok: false,
      error: '技能项链辅助孔必须使用 v103 项目，包括起点、完整未来、目标和未执行指引。',
    }
  const skillVariantAmulets =
    amuletSkillSockets || original.rulesVersion === SKILL_VARIANT_AMULET_RULES_VERSION
  ...
  if (
    !retainedCatalyst &&
    requiresRetainedCatalystProjectVersion(original, catalog, infusedCatalyst)
  )
  ...
  if (native) {
    project.rulesVersion = casterSoulCores
      ? CASTER_SOUL_CORE_RULES_VERSION
      : staffRunes
        ? STAFF_RUNE_RULES_VERSION
        : runeseeker
