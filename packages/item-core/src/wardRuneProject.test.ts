...
const wardId = 'pob2:augment:["Perfect Ward Rune","armour"]'
const version = 'basic-2026-09-16-v83'
...
it('v2–v82 均拒绝未来结界符文、未执行嵌套指引和仅报价', () => {
  ...
  for (let n = 2; n <= 82; n++) {
    ...
    for (const extra of [
      ...
      { pricing: { unit: 'divine', prices: { 'augment:Perfect Ward Rune': 1 } } },
    ])
      expect(read(JSON.stringify({ ...input, ...extra }), real), `v${n}`).toMatchObject({
        ok: false,
        error: expect.stringContaining('v83'),
      })
  }
})
...
const chargingId = 'pob2:augment:["Lesser Charging Rune","armour"]'
...
function socketProject() {
  return {
    ...
    pricing: { unit: 'divine', prices: { 'augment:Perfect Ward Rune': 2 } },
  }
}
it('v83保留全未来覆盖历史、每个游标及报价', () => {
...
it('仅报价与未执行嵌套条件指引触发v83，观察文本不触发能力', () => {
  ...
  for (const input of [
    ...
    expect(requiresWardRuneProjectVersion(input, catalog)).toBe(true)
    expect(must(loadTargetWorkbenchProject(JSON.stringify(input), catalog)).project).toEqual(input)
  }
  expect(requiresWardRuneProjectVersion({ sourceText: JSON.stringify(source) }, catalog)).toBe(
    false,
  )
})
it('沿用新指引升至v83；沿用旧指引不降级、不截断未来、不覆盖报价', () => {
  const strategy = {
    maxSteps: 4,
    rules: [
      {
  ...
  const old = {
    ...socketProject(),
    rulesVersion: 'basic-2026-09-16-v82',
    operations: [],
    pricing: undefined,
  }
...
it('八种真实防具符文覆盖起点、导入声明、未来、嵌套策略和材料名报价', () => {
  const augments =
    catalog.augments?.filter(
      (a) =>
        a.category === 'armour' &&
        /^(?:(?:Lesser|Greater|Perfect) )?(?:Ward|Charging) Rune$/.test(a.name),
    ) ?? []
  expect(augments).toHaveLength(8)
  for (const augment of augments) {
    for (const input of [
      ...
      expect(requiresWardRuneProjectVersion(input, catalog), augment.name).toBe(true)
  }
  ...
  expect(requiresWardRuneProjectVersion(accessor, catalog)).toBe(false)
  const circular: { child?: unknown } = {}
  circular.child = circular
  expect(requiresWardRuneProjectVersion(circular, catalog)).toBe(false)
})
it('v83接收v82锻造指引保留v83并补齐完整签名', () => {
  const loaded = {
    ...catalog,
    runeforging: JSON.parse(readFileSync('data/craft/runeforging.json', 'utf8')),
  }
