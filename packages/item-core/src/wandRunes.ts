...
const records = new Map(WAND_RUNES.map((a) => [a.id, a]))
...
const metadata = WAND_RUNE_SCALABILITY as Record<string, { scalable: boolean; formats: string[] }[]>
...
export function wandRuneFits(
  ...
  return (
    ...
    !base.tags.some((t) => ['twohand', 'staff', 'warstaff', 'not_for_sale'].includes(t)) &&
    !base.hidden &&
    !base.runeforged &&
...
const templates = new Set(WAND_RUNES.flatMap((a) => a.lines.map(template)))
export function isWandRuneLine(line: string): boolean {
  return templates.has(template(line))
}
/** 只有同一属性的可叠加贡献合计；其他条件、负作用与重复行逐行核对。 */
export function wandRuneSourceMatches(
  ...
  const select = (lines: readonly string[]) => {
    ...
    let spellLevels = 0,
      penetration = 0
    for (const line of lines.filter(isWandRuneLine)) {
...
export function wandRuneLimitKey(augment: CatalogAugment): string | null {
  return isWandRune(augment) && augment.limit === 1 ? (augment.limitId ?? augment.name) : null
}
export function wandRuneSocketError(
  catalog: CraftCatalog,
  state: CraftState,
  index: number,
  id: string,
  ...
  const key = augment && wandRuneLimitKey(augment)
  if (!key) return null
  return (state.sockets ?? []).some((old, i) => {
    const other = catalog.augments?.find((a) => a.id === old)
    return i !== index && other && wandRuneLimitKey(other) === key
  })
