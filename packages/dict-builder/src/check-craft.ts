...
if (
  ...
  radiusEmotionMappings.filter((id) => modifierIds.has(id)).length !== 46 ||
  radiusEmotionMappings.filter((id) => excludedJewelIds.has(id)).length !== 0 ||
...
console.log(
  '液态情感声明校验通过：13 普通 / 13 范围材料；普通 50 个映射全部已解析，范围 46 个映射全部已解析，20 个空类别；执行范围见制作规则',
)
