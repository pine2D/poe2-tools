...
扩展 0.1.38 为开发预览版，面向新版 `https://beta.craftofexile.com/` 的 PoE2、English 模式。并非 CoE 官方扩展，不覆盖旧版 www 站、PoE1 或所有英文段落。
...
在 `chrome://extensions/` 打开开发者模式，选择“加载已解压的扩展程序”，加载 `apps/poe2-extension/dist/`。也可把 `apps/poe2-extension/artifacts/poe2-extension-0.1.38.zip` 解压到固定目录后加载。Chrome 不能把这个 ZIP 当商店安装包直接安装。
...
装备导入支持以下基底：无词缀的普通稀有度可直接转换；魔法／稀有装备要求高级分组、等阶和范围完整，支持的词缀如下：
