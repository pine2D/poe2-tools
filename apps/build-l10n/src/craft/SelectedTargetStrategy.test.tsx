...
it('满八组且有残留引用时，先移除旧组再加入新组，配置始终可保存', () => {
  ...
  const props = {
    prefix: '测试条件',
    catalog: catalog(),
    targets: ['p2', 'p3', 'p4', 's1', 's2', 's3', 'p1', 'p2'].map((modId, index) => ({
      targetId: `t${index + 2}`,
      modId,
    })),
  ...
  expect((screen.getByLabelText('测试条件 目标 t9') as HTMLInputElement).disabled).toBe(true)
  fireEvent.click(screen.getByLabelText('测试条件 目标 t9'))
  expect(condition.targetIds).toHaveLength(8)
  fireEvent.click(screen.getByLabelText('测试条件 目标 t1'))
  expect((screen.getByLabelText('测试条件 目标 t9') as HTMLInputElement).disabled).toBe(false)
  fireEvent.click(screen.getByLabelText('测试条件 目标 t9'))
  expect(condition.targetIds).toEqual(['t2', 't3', 't4', 't5', 't6', 't7', 't8', 't9'])
  expect(
    readDefinitionCraftStrategy({
      maxSteps: 20,
