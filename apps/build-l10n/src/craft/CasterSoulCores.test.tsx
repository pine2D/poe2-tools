...
it('恢复空 v127 项目后网页保存不降版', () => {
  ...
  expect(JSON.parse(localStorage.getItem(REHEARSAL_PROJECT_KEY) ?? '{}').rulesVersion).toBe(
    'basic-2026-09-24-v127',
  )
})

it('普通长杖镶入魂核后保存双来源指纹并可恢复', () => {
  ...
  fireEvent.change(screen.getByRole('combobox', { name: '选择镶嵌符文' }), {
    target: { value: 'pob2:augment:["Soul Core of Vizoma","staff"]' },
  })
  ...
  expect(saved.rulesVersion).toBe('basic-2026-09-24-v127')
