...
it('有效目标明确口径并使用合计增效范围', () => {
  ...
  render(
    <TargetValueEditor
      ...
      definitions={{
        nextTargetId: 2,
        targets: [{ targetId: 't1', modId: mod.id }],
        alternatives: [],
        values: [],
      }}
      targetId="t1"
...
it('仅选择增效目标也保存缩放来源，未执行策略仍可恢复', () => {
  ...
  expect(read().rulesVersion).toBe('basic-2026-09-12-v74')
  expect(read().targetDefinitions).toEqual({
    nextTargetId: 2,
    targets: [{ targetId: 't1', modId: 'CraftedJewelPrefixEffect' }],
    alternatives: [],
    values: [],
  })
  expect(read().scalabilitySourceHash).toBe(statScalabilitySourceHash(catalog))
  expect(parseTargetCraftProject(JSON.stringify(read()), catalog).ok).toBe(true)
