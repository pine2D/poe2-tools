import {
  ...
  requiresWandRuneProjectVersion,
...
export function RehearsalPanel({
  ...
  const [runeseekerRules, setRuneseekerRules] = useState(
    initialProject?.project.rulesVersion === RUNESEEKER_RULES_VERSION,
  )
  const [wandRuneRules, setWandRuneRules] = useState(
    initialProject?.project.rulesVersion === WAND_RUNE_RULES_VERSION,
  )
  const [ringLichEchoesRules, setRingLichEchoesRules] = useState(
    initialProject?.project.rulesVersion === RING_LICH_ECHOES_RULES_VERSION,
  )
  const [spendingRules, setSpendingRules] = useState(
    initialProject?.project.rulesVersion === SPENDING_STRATEGY_RULES_VERSION,
  )
  const [tieredMasterworkRules, setTieredMasterworkRules] = useState(
    initialProject?.project.rulesVersion === TIERED_MASTERWORK_RULES_VERSION,
  )
  const [infusedCatalystRules, setInfusedCatalystRules] = useState(
    initialProject?.project.rulesVersion === INFUSED_CATALYST_RULES_VERSION,
  )
  const [blackbloodedEchoesRules, setBlackbloodedEchoesRules] = useState(
    initialProject?.project.rulesVersion === BLACKBLOODED_ECHOES_RULES_VERSION,
  )
  const [ancientRibEchoesRules, setAncientRibEchoesRules] = useState(
    initialProject?.project.rulesVersion === ANCIENT_RIB_ECHOES_RULES_VERSION,
  )
  const [ordinaryAttributeEssenceRules, setOrdinaryAttributeEssenceRules] = useState(
    initialProject?.project.rulesVersion === ORDINARY_ATTRIBUTE_ESSENCE_RULES_VERSION,
  )
  const [jewelVaalRules, setJewelVaalRules] = useState(
    initialProject?.project.rulesVersion === JEWEL_VAAL_RULES_VERSION,
  )
  const [defenceEssenceRules, setDefenceEssenceRules] = useState(
    initialProject?.project.rulesVersion === DEFENCE_ESSENCE_RULES_VERSION,
  )
  const [panelGoalRules, setPanelGoalRules] = useState(
    initialProject?.project.rulesVersion === PANEL_GOAL_RULES_VERSION,
  )
  const [offhandIdolRules, setOffhandIdolRules] = useState(
    initialProject?.project.rulesVersion === OFFHAND_IDOL_RULES_VERSION,
  )
  const [bodyIdolRules, setBodyIdolRules] = useState(
    initialProject?.project.rulesVersion === BODY_IDOL_RULES_VERSION,
  )
  const [helmetBootIdolRules, setHelmetBootIdolRules] = useState(
    initialProject?.project.rulesVersion === HELMET_BOOT_IDOL_RULES_VERSION,
  )
  const [gloveIdolRules, setGloveIdolRules] = useState(
    initialProject?.project.rulesVersion === GLOVE_IDOL_RULES_VERSION,
  )
  const [sceptreAugmentRules, setSceptreAugmentRules] = useState(
    initialProject?.project.rulesVersion === SCEPTRE_AUGMENT_RULES_VERSION,
  )
  ...
  const [amuletSkillLevelRules, setAmuletSkillLevelRules] = useState(
    initialProject?.project.rulesVersion === AMULET_SKILL_LEVEL_RULES_VERSION,
  )
  const [amuletSkillSocketRules, setAmuletSkillSocketRules] = useState(
    initialProject?.project.rulesVersion === AMULET_SKILL_SOCKETS_RULES_VERSION,
  )
  const [skillVariantAmuletRules, setSkillVariantAmuletRules] = useState(
    initialProject?.project.rulesVersion === SKILL_VARIANT_AMULET_RULES_VERSION,
  )
  ...
  const retainedCatalystHistory = requiresRetainedCatalystProjectVersion(
    { history },
    catalog,
    ringLichEchoesRules ||
      spendingRules ||
      tieredMasterworkRules ||
      infusedCatalystRules ||
      requiresInfusedCatalystProjectVersion({ history }, catalog),
  )
  const hasEssenceHistory =
    requiresOrdinaryAttributeEssenceProjectVersion({ history }) ||
    requiresDefenceEssenceProjectVersion({ history }) ||
  ...
  const emotionHash =
    ...
    history.some(
      ({ state }) =>
        usesJewelCapacity(catalog, state) && !isOrdinaryCorruptedJewelCapacity(catalog, state),
    ) ||
  ...
  const needsRuneseeker = runeseekerRules || requiresRuneseekerProjectVersion(projectConfig)
  const usesWandRunes = requiresWandRuneProjectVersion(projectConfig)
  const needsWandRunes = needsRuneseeker || wandRuneRules || usesWandRunes
  const needsRingLichEchoes =
    ringLichEchoesRules || requiresRingLichEchoesProjectVersion(projectConfig, catalog)
  const needsSpending = spendingRules || requiresSpendingProjectVersion(projectConfig)
  const needsTieredMasterwork =
    tieredMasterworkRules ||
    requiresTieredMasterworkProjectVersion(projectConfig, catalog) ||
    requiresTieredMasterworkProjectVersion({ history, strategy }, catalog)
  const needsInfusedCatalyst =
    infusedCatalystRules ||
    requiresInfusedCatalystProjectVersion(projectConfig, catalog) ||
    requiresInfusedCatalystProjectVersion({ history }, catalog)
  const needsBlackbloodedEchoes =
    blackbloodedEchoesRules ||
    requiresBlackbloodedEchoesProjectVersion(projectConfig) ||
    requiresBlackbloodedEchoesProjectVersion({ history })
  const needsAncientRibEchoes =
    ancientRibEchoesRules ||
    requiresAncientRibEchoesProjectVersion(projectConfig) ||
    requiresAncientRibEchoesProjectVersion({ history })
  const needsOrdinaryAttributeEssences =
    ordinaryAttributeEssenceRules ||
    requiresOrdinaryAttributeEssenceProjectVersion(projectConfig) ||
    requiresOrdinaryAttributeEssenceProjectVersion({ history })
  const needsJewelVaal =
    jewelVaalRules ||
    requiresJewelVaalProjectVersion(projectConfig) ||
    requiresJewelVaalProjectVersion({ history }) ||
    history.some(({ state }) => isOrdinaryCorruptedJewelCapacity(catalog, state))
  const needsDefenceEssences =
    defenceEssenceRules ||
    requiresDefenceEssenceProjectVersion(projectConfig) ||
    requiresDefenceEssenceProjectVersion({ history })
  const needsPanelGoals = panelGoalRules || requiresPanelGoalProjectVersion(projectConfig)
  const needsOffhandIdols = offhandIdolRules || requiresOffhandIdolProjectVersion(projectConfig)
  const needsBodyIdols = bodyIdolRules || requiresBodyIdolProjectVersion(projectConfig)
  const needsHelmetBootIdols =
    helmetBootIdolRules || requiresHelmetBootIdolProjectVersion(projectConfig)
  const needsGloveIdols = gloveIdolRules || requiresGloveIdolProjectVersion(projectConfig)
  const needsSceptreAugments =
    sceptreAugmentRules || requiresSceptreAugmentProjectVersion(projectConfig)
  ...
  const needsAmuletSkillLevel =
    amuletSkillLevelRules || requiresAmuletSkillLevelProjectVersion(projectConfig)
  const needsAmuletSkillSockets =
    amuletSkillSocketRules || requiresAmuletSkillSocketsProjectVersion(projectConfig)
  const needsSkillVariantAmulet =
    skillVariantAmuletRules || requiresSkillVariantAmuletProjectVersion(projectConfig)
    ...
    needsRuneforge
      ? {
          ...
          ...((usesWandRunes ||
            ...
            requiresHelmetBootIdolProjectVersion(projectConfig) ||
            requiresGloveIdolProjectVersion(projectConfig) ||
            requiresSceptreAugmentProjectVersion(projectConfig) ||
            requiresSpecialMartialRuneProjectVersion(projectConfig)) &&
          augmentSourceHash
            ? {
                augmentSourceHash,
                scalabilitySourceHash: statScalabilitySourceHash(catalog) as string,
  ...
  const restoreProject = (restored: RestoredTargetCraftProject) => {
    ...
    setRingLichEchoesRules(restored.project.rulesVersion === RING_LICH_ECHOES_RULES_VERSION)
    setSpendingRules(restored.project.rulesVersion === SPENDING_STRATEGY_RULES_VERSION)
    setTieredMasterworkRules(restored.project.rulesVersion === TIERED_MASTERWORK_RULES_VERSION)
    setInfusedCatalystRules(restored.project.rulesVersion === INFUSED_CATALYST_RULES_VERSION)
    setBlackbloodedEchoesRules(restored.project.rulesVersion === BLACKBLOODED_ECHOES_RULES_VERSION)
    setAncientRibEchoesRules(restored.project.rulesVersion === ANCIENT_RIB_ECHOES_RULES_VERSION)
    setOrdinaryAttributeEssenceRules(
      restored.project.rulesVersion === ORDINARY_ATTRIBUTE_ESSENCE_RULES_VERSION,
    )
    setJewelVaalRules(restored.project.rulesVersion === JEWEL_VAAL_RULES_VERSION)
    setDefenceEssenceRules(restored.project.rulesVersion === DEFENCE_ESSENCE_RULES_VERSION)
    setPanelGoalRules(restored.project.rulesVersion === PANEL_GOAL_RULES_VERSION)
    setOffhandIdolRules(restored.project.rulesVersion === OFFHAND_IDOL_RULES_VERSION)
    setBodyIdolRules(restored.project.rulesVersion === BODY_IDOL_RULES_VERSION)
    setHelmetBootIdolRules(restored.project.rulesVersion === HELMET_BOOT_IDOL_RULES_VERSION)
    setGloveIdolRules(restored.project.rulesVersion === GLOVE_IDOL_RULES_VERSION)
    setSceptreAugmentRules(restored.project.rulesVersion === SCEPTRE_AUGMENT_RULES_VERSION)
    ...
    setSkillVariantAmuletRules(restored.project.rulesVersion === SKILL_VARIANT_AMULET_RULES_VERSION)
        ...
        onExtract={(targets) => {
          const changed = editTargetDefinitionContext(
            ...
            targetContext,
            {
              ...
              definitions: {
                ...targets,
                ...(targetContext.definitions.panelGoals === undefined
                  ? {}
                  : { panelGoals: targetContext.definitions.panelGoals }),
              },
            },
            targetCapacityContext,
